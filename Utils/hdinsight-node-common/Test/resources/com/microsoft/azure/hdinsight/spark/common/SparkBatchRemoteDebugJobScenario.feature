Feature: Spark Batch Remote Debug Job Testing
  ...
  Scenario: getSparkJobDriverLogUrlObservable unit test with mode selection
    Given mock getSparkJobYarnCurrentAppAttempt with the following response:
      | logsLink | http://10.0.0.4:8042/node/containerlogs/container_1326821518301_0005_01_000001/user1 |
    And mock Spark job uri 'https://cluster/yarnui/10.0.0.4/node/containerlogs/container_1326821518301_0005_01_000001/user1' is invalid
    And mock Spark job uri 'https://cluster/yarnui/10.0.0.4/port/8042/node/containerlogs/container_1326821518301_0005_01_000001/user1' is valid
    And mock Spark job connect URI to be 'https://cluster/'
